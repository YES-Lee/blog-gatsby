...
graphql会根据请求中声明的字段来执行resolver，一定程度上可以减少查询次数。下面的代码中，会执行`UserResolver`，结束后再继续执行`RoleResolver`
...
其中需要注意的是，敏感数据不应该出现在敏感信息（用户密码等），即使Resolver的结果包含这些敏感信息，只要`schema`中没有包含，graphql会自动过滤这些字段。
