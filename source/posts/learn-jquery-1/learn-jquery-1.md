...
`BookPrototype`为`Book`类的原型（基类），使用`new`关键字创建的`Book`实例，都会包含`BookPrototype`里面的属性和方法。这里刚开始然我有些迷惑，参考经典的面向对象语言，基类应该也是一个类，这里却是一个对象字面量。首先，需要深刻明白一点: JavaScript中，一切皆是对象。比如一个函数，我们可以给它加上任何的属性
