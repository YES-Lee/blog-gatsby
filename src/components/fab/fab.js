...
export default function Fab (props) {
  ...
  return (
    <div className={styles.fab}>
      <div className={`${styles.fabMenuGroup} ${showMenu ? styles.fabActive : ''}`}>
        <span className={`${styles.fabMenu} icon-chevron-up`} onClick={() => act('top')}></span>
        <span className={`${styles.fabMenu} icon-chevron-down`} onClick={() => act('bottom')}></span>
        {
          fabSider && (
            <span className={`${styles.fabMenu} icon-sidebar`} onClick={() => act('sider')}></span>
          )
        }
        {
          fabToc && (
            <span className={`${styles.fabMenu} icon-list`} onClick={() => act('toc')}></span>
          )
        }
      ...
      <div className={styles.fabControl} onClick={() => setShowMenu(!showMenu)}>
        <svg className={styles.fabControlSvg}>
          ...
          <circle className={styles.fabControlCover} cx="50%" cy="50%" r="44%"></circle>
          <circle
